...
from pydepgraph.exceptions import PDGMissingModuleNameError, PDGSourceFileOutsideProjectError
from pydepgraph.parser import validate_python_file
...
class ModuleSource(Specification):
    ...
    def relative(self) -> ImportPath:
        ...
        if relative is None:
            raise PDGSourceFileOutsideProjectError(f"File '{self.origin}' is outside the project: '{self.base_path}'")
    ...
    def top_level(self) -> str:
        ...
        if not module:
            raise PDGMissingModuleNameError(
    ...
    def get_spec(
        ...
        if module_name is None:
            raise PDGMissingModuleNameError(f"Import path '{path}' does not specify a module name")
