...
from pydepgraph.exceptions import PDGPathResolutionError, PDGRelativeBasePathError
from pydepgraph.specification.imports import ImportPath
from pydepgraph.specification.modules.spec import validate_spec_origin
...
class SysPaths(metaclass=Singleton):
    ...
    def get_candidates(cls, base_path: Optional[Pathlike] = None) -> List[Path]:
        ...
        if base_path is not None:
            ...
            if not base_path.is_absolute():
                raise PDGRelativeBasePathError(f"Base path must be an absolute path, got: {base_path}")
    ...
    def resolve(
        ...
        if not origin.is_absolute():
            raise PDGPathResolutionError(f"Provided path must be absolute, got: {origin}")
        ...
        raise PDGPathResolutionError(f"Could not resolve path '{origin}' against candidates: {candidates}")
